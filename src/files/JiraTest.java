...
public class JiraTest {
	...
	public static void main(String[] args) {
		...
		String response = given().header("Content-Type", "application/json")
				.body("{\r\n" + "  \"username\": \"hcnn1\",\r\n" + "  \"password\": \"!Vpanfil0v74\"\r\n" + "}").log()
				.all().filter(session).when().post("/rest/auth/1/session").then().log().all().extract().response()
				.asString();

		String expectedMessage = "This is expected message for Add comment API test";
		// Add comment
